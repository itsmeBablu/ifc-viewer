...
export function snapElevatedEndpoints<T extends { xMm: number; yMm: number; worldElevationMm: number }>(points: T[], options: Pick<GlobalSnapOptions, "camera" | "canvas" | "clientPos" | "tolerancePx" | "activeModes">): T | null {
  ...
  for (const point of points) {
    ...
    const screen = worldMmToScreen(point.xMm, point.yMm, point.worldElevationMm, options.camera, options.canvas);
    const d = Math.hypot(screen.clientX - options.clientPos.x, screen.clientY - options.clientPos.y);

...
}).strict().describe("Complete unfurnished concept apartment: 1–6 bedrooms (default 20 m² clear each), 1.2 m corridor, living/open kitchen, 8 m² bathroom, slab, doors and windows. Code computes geometry. No roof or MEP. IDs id:wall:0 onwards. Use for routine apartment briefs without a bespoke footprint.");

export function apartmentActions(item: z.infer<typeof apartmentSchema>, options: { bays?: number; serviceDepthMm?: number; entrance?: boolean } = {}): AiAction[] {
  const actions: AiAction[] = [];
  const t = item.thicknessMm, p = 150, roomW = 4000, roomD = item.bedroomAreaM2 * 1e6 / roomW;
  const bays = options.bays ?? item.bedrooms;
  const w = bays * roomW + (bays - 1) * p + t;
  const bedroomEnd = t / 2 + roomD + p / 2;
  const corridorEnd = bedroomEnd + p + 1200;
  const d = corridorEnd + p / 2 + (options.serviceDepthMm ?? 4000) + t / 2;

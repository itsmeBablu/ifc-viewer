...
export function multiApartmentActions(input: ResidentialParameters, id: string, groundId: string, baseElevationMm: number, heightMm: number, thicknessMm: number): AiAction[] {
  ...
  const coreWidth = (hasLift || hasStairs) ? 3400 : 0;
  ...
  for (let floor = 0; floor < floors; floor++) {
    ...
    // Complete continuous floor plate covering all units and circulation core
    actions.push({
      kind: "floor",
      operation: "create",
      id: `${id}:floor:${floor}`,
      levelId,
      boundary: [{ xMm: 0, yMm: 0 }, { xMm: totalWidth + coreWidth, yMm: 0 }, { xMm: totalWidth + coreWidth, yMm: depth }, { xMm: 0, yMm: depth }],
      thicknessMm: 200,
      elevationOffsetMm: 0,
      roofPreset: "flat",
      pitchDeg: 0,
    ...
    // Optional Shared Passenger Lift (aligned vertically on every floor)
    if (hasLift) {
      actions.push({
        kind: "equipment",
        operation: "create",
    ...
    // Optional Common Staircase Landing Core (aligned vertically between storeys)
    if (hasStairs && floor < floors - 1) {
      actions.push({
        ...
        levelId,
        boundary: [{ xMm: totalWidth + 400, yMm: 600 }, { xMm: totalWidth + 3000, yMm: 600 }, { xMm: totalWidth + 3000, yMm: 3600 }, { xMm: totalWidth + 400, yMm: 3600 }],
        thicknessMm: 180,
        elevationOffsetMm: 0,
        roofPreset: "flat",
        pitchDeg: 0,

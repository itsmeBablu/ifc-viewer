...
export function houseActions(item: z.infer<typeof houseSchema>): AiAction[] {
  ...
  for (let floor = 0; floor < floors; floor++) {
    ...
    let layout = apartmentActions(apartmentSchema.parse({ kind: "apartment_layout", id: prefix, levelId, bedrooms: Math.max(1,bedrooms), bedroomAreaM2: item.bedroomAreaM2, xMm: item.xMm, yMm: item.yMm, heightMm: item.heightMm, thicknessMm: item.thicknessMm, furnished: false, balcony: item.balcony, layoutSeed: item.layoutSeed !== undefined ? (item.layoutSeed + floor * 101) : undefined, separateKitchen: item.separateKitchen, ensuiteBathrooms: item.ensuiteBathrooms, bathroomCount: Math.max(1, floor ? Math.max(1, Math.floor((item.bathroomCount ?? floors) / floors)) : Math.ceil((item.bathroomCount ?? floors) / floors)), guestBathroom: item.guestBathroom && !floor, guestBathroomShower: item.guestBathroomShower, bedroomTypes: item.bedroomTypes?.slice(floor ? groundBedrooms : 0, floor ? item.bedrooms : groundBedrooms), bedroomEnsuites: item.bedroomEnsuites?.slice(floor ? groundBedrooms : 0, floor ? item.bedrooms : groundBedrooms), bedroomAreasM2: item.bedroomAreasM2?.slice(floor ? groundBedrooms : 0, floor ? item.bedrooms : groundBedrooms) }), { allocation, entrance: !floor });
    layout.push(...furnishFloor({ ...item, layoutSeed: item.layoutSeed !== undefined ? item.layoutSeed + floor * 101 : undefined, guestBathroom: item.guestBathroom && !floor, guestBathroomShower: item.guestBathroomShower, bedroomTypes: item.bedroomTypes?.slice(floor ? groundBedrooms : 0, floor ? item.bedrooms : groundBedrooms), bedroomEnsuites: item.bedroomEnsuites?.slice(floor ? groundBedrooms : 0, floor ? item.bedrooms : groundBedrooms), bedroomAreasM2: item.bedroomAreasM2?.slice(floor ? groundBedrooms : 0, floor ? item.bedrooms : groundBedrooms) },allocation,prefix,levelId,item.xMm,item.yMm,item.thicknessMm,item.heightMm,bedrooms,Boolean(floor)));
    ...
    if (floor === floors - 1) {
      const requestedPreset = item.roofStyle === "modern-flat" ? "flat" : item.roofStyle === "german-gable" ? "gable" : item.roofStyle === "mansard" ? "shed" : item.roofStyle === "german-hip" ? "hip" : (item.variant === "villa" && !building.polygon ? "gable" : "flat");
      const roofPreset = (building.polygon && building.polygon.length !== 4) ? "flat" : requestedPreset;

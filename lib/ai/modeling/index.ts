...
export function defaultModelingPlan(input: CommandRequest) {
  ...
  if(brief.sketches){
    ...
    if(brief.totalAreaM2!==undefined&&Math.abs(area-brief.totalAreaM2)>.1)throw new Error(`Drawn floors give ${area.toFixed(1)} mÂ². Update the total area or adjust the drawn lines.`);
    ...
    const plan=validatePlan({summary:`Sketch-based ${brief.variant}: ${brief.sketches.length} floor(s), ${area.toFixed(1)} mÂ² internal area.`,assumptions:["Drawn outside and inside lines determine each floor; explicit drawing dimensions take precedence over automatic room areas.","Outside-only floors receive automatic rooms; interior partitions determine enclosed rooms and furniture placement.","Furniture follows walls and room groups with circulation and stair clearances.","Multi-storey homes include concept U-stairs and aligned floor openings; garage and garden are additional."],actions:[...(input.context.activeLevelId?[]:[{kind:"level",operation:"create",id:levelId,name:"Ground",elevationMm:0,heightMm:height}]),...actions]},input.context);
    return {kind:"plan"as const,plan,model:input.model??"gemini-3.1-flash-lite",mode:"build"as const,usage:{inputTokens:0,outputTokens:0,thinkingTokens:0}};
  }

...
export function describeAction(action: AiAction) {
  ...
  if (action.kind === "cabletray" || action.kind === "wall" || action.kind === "beam") return `${verb} ${action.kind} ${action.id}: (${action.startXmm}, ${action.startYmm}) → (${action.endXmm}, ${action.endYmm}) mm, level ${action.levelId}`;
...
export function validatePlan(value: unknown, rawContext: AiContext) {
  ...
  for (const action of plan.actions) {
    ...
    if (action.kind === "cabletray" || action.kind === "wall" || action.kind === "beam" || action.kind === "duct" || action.kind === "pipe") {

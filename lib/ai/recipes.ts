...
export const recipeSchema = z.discriminatedUnion("kind", [
  apartmentSchema,
  houseSchema,
  z.object({kind:z.literal("apartment_block"),id,levelId:wallFields.levelId,floors:z.number().int().min(1).max(12),unitsPerFloor:z.number().int().min(2).max(10),bedroomsPerUnit:z.union([z.literal(1),z.literal(2),z.literal(3)]),bedroomDistribution:z.array(z.union([z.literal(1),z.literal(2),z.literal(3)])).min(1).max(100).optional(),totalAreaM2:z.number().min(30).max(20000).optional(),heightMm:wallFields.heightMm.default(3000),thicknessMm:wallFields.thicknessMm.default(200),furnished:z.boolean().default(true),separateKitchen:z.boolean().default(true),ensuiteBathrooms:z.boolean().default(true),commonStairs:z.boolean().optional(),lift:z.boolean().optional(),curtainFacade:z.boolean().optional(),electrical:z.boolean().optional(),underfloorHeating:z.boolean().optional(),piping:z.enum(["none","underfloor","ceiling"]).optional(),ducts:z.enum(["none","ceiling"]).optional()}).strict().describe("Create a multi-storey apartment block with repeated independent homes, separate kitchens, ensuite bathrooms, optional shared lift, common stairs, MEP, and curtain facade."),
  z.object({
...
export function expandModelPlan(value: unknown) {
  ...
  for (const item of plan.actions) {
    switch (item.kind) {
      ...
      case "apartment_block": multiApartmentActions({variant:"apartment",bedrooms:item.bedroomsPerUnit,apartmentFloors:item.floors,apartmentsPerFloor:item.unitsPerFloor,bedroomsPerApartment:item.bedroomsPerUnit,bedroomDistribution:item.bedroomDistribution,totalAreaM2:item.totalAreaM2,furnished:item.furnished,separateKitchen:item.separateKitchen,ensuiteBathrooms:item.ensuiteBathrooms,commonStairs:item.commonStairs,lift:item.lift,curtainFacade:item.curtainFacade,electrical:item.electrical,underfloorHeating:item.underfloorHeating,piping:item.piping,ducts:item.ducts},item.id,item.levelId,0,item.heightMm,item.thicknessMm).forEach(append); break;

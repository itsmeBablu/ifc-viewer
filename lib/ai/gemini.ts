  ...
  const response = await fetch("https://generativelanguage.googleapis.com/v1beta/models/gemini-3.6-flash:generateContent", {
    ...
    body: JSON.stringify({
      ...
      contents: [
        ...
        { role: "user", parts: [{ text: JSON.stringify({ command: input.command, project: input.context, attachedFiles: input.attachments.map(file => file.name) }) }, ...input.attachments.map(file => ({ inlineData: { mimeType: file.mimeType, data: file.data } }))] },
      ],

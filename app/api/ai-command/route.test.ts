...
describe("AI command authorization", () => {
  ...
  it("sends wizard layout requests to Gemini instead of bypassing design interpretation", async () => {
    vi.mocked(auth).mockResolvedValue({ user: { id: "google-123" } } as never);
    vi.mocked(generateCommand).mockResolvedValue({ kind: "layout", parameters: { variant: "villa", bedrooms: 3, footprint: "u", layoutSeed: 91234, layoutRevision: 0, layoutStyle: "courtyard" }, message: "Courtyard concept", model: "gemini-3.1-flash-lite", mode: "build" });

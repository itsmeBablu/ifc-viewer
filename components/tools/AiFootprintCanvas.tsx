...
export default function AiFootprintCanvas({parameters,onChange,disabled,building}:{parameters:ResidentialParameters;onChange:(p:ResidentialParameters)=>void;disabled:boolean;building?:ReturnType<typeof import("@/lib/ai/modeling/footprint").allocateBuilding>|null}){
  const shape=parameters.footprint??"rectangle",points=shape==="drawn"?parameters.footprintPoints??[]:FOOTPRINTS[shape];
  ...
  const add=(e:React.MouseEvent<SVGSVGElement>)=>{
    if(shape!=="drawn"||disabled||points.length>=16)return;
  ...
  return <div className="ai-footprint-chooser">
    <div className="ai-suggestion-row">{(["rectangle","l","u","drawn"]as const).map(s=><button key={s} type="button" disabled={disabled} aria-pressed={shape===s} onClick={()=>onChange({...parameters,footprint:s})}>{s==="rectangle"?"Rectangle":s==="l"?"L shape":s==="u"?"U shape":"Draw outline"}</button>)}</div>

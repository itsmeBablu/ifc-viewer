...
export default function AiFootprintCanvas({parameters,onChange,disabled,building}:{parameters:ResidentialParameters;onChange:(p:ResidentialParameters)=>void;disabled:boolean;building?:ReturnType<typeof import("@/lib/ai/modeling/footprint").allocateBuilding>|null}){
  ...
  const bx=building?.block?.xMm??0,by=building?.block?.yMm??0; const previewFactors=allocation?Array.from({length:allocation.bays},(_,i)=>.82+((((parameters.layoutSeed??0)*17+i*31)%37)/100)):[]; const previewTotal=previewFactors.reduce((a,v)=>a+v,0); const previewWidths=allocation?previewFactors.map(v=>allocation.roomWidthMm*allocation.bays*v/previewTotal):[];
  return <div className="ai-footprint-chooser">
    ...
    <svg className="ai-footprint-canvas" viewBox="0 0 120 120" role="img" aria-label={shape==="drawn"?"Draw building footprint by clicking corners":"Building footprint preview"} onClick={add}>
      ...
      {allocation&&shape!=="drawn"&&<g>{Array.from({length:allocation.bays},(_,i)=>{const offset=previewWidths.slice(0,i).reduce((a,v)=>a+v+150,0),x=10+(bx+offset)/w*100,y=10+by/d*100;return <g key={i}><rect x={x} y={y} width={(previewWidths[i]??allocation.roomWidthMm)/w*100} height={allocation.roomDepthMm/d*100} fill="#60a5fa66" stroke="currentColor" strokeWidth=".25"/><text x={x+(previewWidths[i]??allocation.roomWidthMm)/w*50} y={y+allocation.roomDepthMm/d*50} textAnchor="middle" fontSize="4" fill="currentColor">Bed / study</text></g>;})}<text x={10+(bx+allocation.internalWidthMm/2)/w*100} y={10+(by+allocation.roomDepthMm+750)/d*100} textAnchor="middle" fontSize="4" fill="currentColor">Corridor</text><rect x={10+bx/w*100} y={10+(by+allocation.roomDepthMm+1500)/d*100} width={allocation.bathroomWidthMm/w*100} height={allocation.bathroomDepthMm/d*100} fill="#c084fc55"/><text x={10+(bx+allocation.bathroomWidthMm+allocation.internalWidthMm)/2/w*100} y={10+(by+allocation.internalDepthMm-1000)/d*100} fontSize="4" textAnchor="middle" fill="currentColor">Living / kitchen</text></g>}

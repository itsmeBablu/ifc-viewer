...
export default function WerkzeugWorkspaceChrome({
  ...
  const [auxOpen, setAuxOpen] = useState<"levels" | "views" | "scale" | "elements" | null>(null);
  const [lastAux, setLastAux] = useState<"levels" | "views" | "scale" | "elements">("views");
  ...
  useEffect(() => {
    const updateOrientation = () => {
      ...
      setLandscapePanelHeight((height) =>
        Math.max(180, Math.min(height, window.innerHeight * 0.8)),
      );
  ...
  useLayoutEffect(() => {
    ...
    const measureContent = () => {
      ...
      const preferredWidth = panelKey === "materials"
        ? 380
        : panelKey === "levels" || panelKey === "column" || panelKey === "beam"
          ? 350
          : 320;
      const widthLimit = Math.min(420, window.innerWidth - 16);
      const targetWidth = Math.max(
        Math.min(280, widthLimit),
        Math.min(widthLimit, preferredWidth),
      );
  ...
  const toggleAux = (next: "levels" | "views" | "scale" | "elements", anchor: HTMLElement) => {
    setRenderOpen(false);
  ...
  return (
    <>
      ...
      >
        ...
        <div className="flex h-7 shrink-0 items-center pr-1" title="IBV Viewer">
      ...
      {panelKey && <div ref={panelRef} data-orientation={portrait ? "portrait" : "landscape"} data-hidden={panelHidden ? "true" : "false"} className="werkzeug-ipad-context pointer-events-auto fixed z-[68]" style={portrait ? { right: 8, bottom: 8, width: portraitPanelWidth, maxWidth: "calc(100vw - 16px)", height: collapsed ? 48 : portraitPanelHeight } : { right: 8, bottom: 8, width: landscapePanelWidth, maxWidth: "calc(100vw - 16px)", height: collapsed ? 48 : landscapePanelHeight }}>
        ...
        <GlassPanel variant="panel" zIndex={68} fill preferCss wrapperClassName="werkzeug-ipad-context-surface h-full overflow-hidden rounded-xl">
          <div className="werkzeug-ipad-drawer-body flex h-full min-h-0 flex-col">
            <div onPointerDown={portrait ? beginDrag : undefined} className={`flex min-h-10 shrink-0 touch-none items-center gap-2 border-b border-[var(--panel-divider)] px-2.5 ${portrait ? "cursor-move" : ""}`}>
              <span className="shrink-0 text-[11px] font-semibold capitalize text-[var(--text-strong)]">{[...TOOL_ITEMS, ...MEP_TOOL_ITEMS].find((item) => item.id === panelKey)?.label ?? panelKey} options</span>
              <div className="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden" onPointerDown={e => e.stopPropagation()}>
                {(["properties", "type", "materials", "layout"] as const).map(tab => <button key={tab} type="button" aria-pressed={panelTab === tab} onClick={() => { setPanelTab(tab); setPanelHidden(false); }} className={`min-h-8 shrink-0 rounded-md border px-2 text-[10px] font-semibold capitalize ${panelTab === tab ? "border-amber-400 bg-amber-400/15 text-amber-300" : "border-transparent text-[var(--text-muted)] hover:border-[var(--panel-divider)]"}`}>{tab === "materials" ? "Materials" : tab}</button>)}
              </div>
              <div className="flex shrink-0 items-center gap-1">
            ...
            {!collapsed && <div ref={contentRef} className="werkzeug-ipad-panel-content min-h-0 flex-1 overflow-y-auto p-2 thin-scroll">{desktopCategory === "render" ? <RenderViewControls /> : <>{((panelKey === "lines" && armed === "lines") || (panelKey === "floor" && armed === "floor") || (panelKey === "ceiling" && armed === "ceiling") || (panelKey === "roof" && armed === "roof") || (panelKey === "wall" && armed === "wall")) && <DrawingShapeOptions />}{panelKey === "levels" || panelTab === "layout" ? <LevelsPanel /> : panelKey === "materials" || panelTab === "materials" ? <MaterialEditorPanel isOpen embedded onClose={() => panelKey === "materials" ? setPanelHidden(true) : setPanelTab("properties")} /> : panelKey === "shapes" ? <MarkupToolsSection /> : <ToolContent panelKey={panelKey} locked={locked} tab={panelTab} />}</>}</div>}
            {!collapsed && <>
...
function TypeOptions({ panelKey, locked }: { panelKey: LayoutToolId; locked: boolean }) {
  ...
  const selectedWall = layout.walls.find(w => w.id === layout.selectedWallId);
  const selectedDoor = layout.doors.find(d => d.id === layout.selectedDoorId);
  const selectedWindow = layout.windows.find(w => w.id === layout.selectedWindowId);
  const presets: Array<{ name: string; thickness?: number; width?: number; height?: number; sill?: number }> = panelKey === "door"
      ? [{ name: "Single 800", width: 800, height: 2100 }, { name: "Single 900", width: 900, height: 2100 }, { name: "Double 1800", width: 1800, height: 2100 }]
      : panelKey === "window"
        ? [{ name: "Single 900", width: 900, height: 1200, sill: 900 }, { name: "Double 1200", width: 1200, height: 1400, sill: 850 }, { name: "Wide 1800", width: 1800, height: 1400, sill: 750 }]
  ...
  const apply = (preset: typeof presets[number]) => {
    ...
    if (panelKey === "window" && layout.selectedWindowId && preset.width && preset.height) void layout.updateWindow(layout.selectedWindowId, { widthMm: preset.width, heightMm: preset.height, sillHeightMm: preset.sill ?? 900 });
  ...
  const field = "h-9 w-full rounded-lg border border-[var(--panel-divider)] bg-transparent px-2 text-[11px] text-[var(--text-strong)]";
  const current = panelKey === "door" ? selectedDoor : panelKey === "window" ? selectedWindow : selectedWall;
  const openingCurrent = current as (typeof selectedDoor | typeof selectedWindow | null);
  return <div className="space-y-3">
    <div><p className="text-[10px] font-semibold uppercase tracking-wide text-[var(--text-muted)]">Default types</p><p className="mt-1 text-[10px] text-[var(--text-muted)]">Tap a preset to apply it. All dimensions remain editable below.</p></div>
    <div className="grid grid-cols-1 gap-1.5">{presets.map((preset) => <button disabled={locked} type="button" key={preset.name} onClick={() => apply(preset)} className={`${compactButton} flex w-full items-center justify-between bg-transparent text-left`}><span>{preset.name}</span><span className="text-[10px] text-[var(--text-muted)]">{preset.width ? `${preset.width} × ${preset.height} mm` : `${preset.thickness} mm`}</span></button>)}</div>
    {panelKey === "wall" && <div className="space-y-2"><label className="text-[9px] font-semibold text-[var(--text-muted)]">Wall type<select className={field} disabled={locked} value={selectedWall?.wallTypeId ?? layout.draftWallTypeId ?? ""} onChange={e => selectedWall ? void layout.updateWall(selectedWall.id, { wallTypeId: e.target.value }) : layout.setDraftWallTypeId(e.target.value)}><option value="">Generic wall</option>{layout.wallTypes.map(type => <option key={type.id} value={type.id}>{type.name} · {type.totalThicknessMm} mm</option>)}</select></label><div className="grid grid-cols-2 gap-2"><label className="text-[9px] font-semibold text-[var(--text-muted)]">Thickness (mm)<input className={field} disabled={locked} type="number" value={selectedWall?.thicknessMm ?? layout.draftWallThicknessMm} onChange={e => selectedWall ? void layout.updateWall(selectedWall.id, { thicknessMm: Number(e.target.value) }) : layout.setDraftWallThicknessMm(Number(e.target.value))} /></label><label className="text-[9px] font-semibold text-[var(--text-muted)]">Height (mm)<input className={field} disabled={locked} type="number" value={selectedWall?.heightMm ?? layout.draftWallHeightMm} onChange={e => selectedWall ? void layout.updateWall(selectedWall.id, { heightMm: Number(e.target.value) }) : layout.setDraftWallHeightMm(Number(e.target.value))} /></label></div></div>}
    {(panelKey === "door" || panelKey === "window") && openingCurrent && <div className="grid grid-cols-2 gap-2">{(["widthMm", "heightMm"] as const).map(key => <label key={key} className="text-[9px] font-semibold text-[var(--text-muted)]">{key === "widthMm" ? "Width" : "Height"} (mm)<input className={field} disabled={locked} type="number" value={openingCurrent[key]} onChange={e => panelKey === "door" ? void layout.updateDoor(openingCurrent.id, { [key]: Number(e.target.value) }) : void layout.updateWindow(openingCurrent.id, { [key]: Number(e.target.value) })} /></label>)}{panelKey === "window" && <label className="text-[9px] font-semibold text-[var(--text-muted)] col-span-2">Sill height (mm)<input className={field} disabled={locked} type="number" value={selectedWindow?.sillHeightMm ?? 900} onChange={e => void layout.updateWindow(openingCurrent.id, { sillHeightMm: Number(e.target.value) })} /></label>}{panelKey === "door" && selectedDoor && <label className="text-[9px] font-semibold text-[var(--text-muted)] col-span-2">Position along host wall (mm)<input className={field} disabled={locked} type="number" value={selectedDoor.positionMm} onChange={e => void layout.updateDoor(selectedDoor.id, { positionMm: Number(e.target.value) })} /></label>}{panelKey === "window" && selectedWindow && <label className="text-[9px] font-semibold text-[var(--text-muted)] col-span-2">Position along host wall (mm)<input className={field} disabled={locked} type="number" value={selectedWindow.positionMm} onChange={e => void layout.updateWindow(selectedWindow.id, { positionMm: Number(e.target.value) })} /></label>}</div>}
    {!current && <p className="rounded-lg border border-amber-400/30 bg-amber-400/10 p-2 text-[10px] text-amber-200">Select an existing {panelKey} to edit its full type and dimensions.</p>}
  </div>;
}

...
export default function AiPlanPreview({ plan, context }: { plan: AiPlan; context: AiContext }) {
  ...
  return <div>{levels.map(levelId => <figure key={levelId} className="my-2">
    ...
    <svg aria-label={`Proposed geometry on level ${levelId}`} role="img" viewBox={`${minX-margin} ${minY-margin} ${width+margin*2} ${height+margin*2}`} className="mt-1 h-40 w-full rounded-lg ai-plan-canvas">
      {surfaces.filter(s => s.levelId === levelId).map(s => <polygon key={s.id} points={s.boundary.map(p => `${p.xMm},${p.yMm}`).join(" ")} fill={s.kind === "roof" ? "#60a5fa22" : "#facc1522"} stroke="var(--text-muted)" strokeWidth={Math.max(width, height)/300} />)}
      {lines.filter(l => l.levelId === levelId).map(l => <line key={l.id} x1={l.x1} y1={l.y1} x2={l.x2} y2={l.y2} stroke="var(--ai-accent-ink)" strokeWidth={Math.max(width, height)/150} />)}
    </svg>
  </figure>)}<p className="text-xs ai-text-muted">Schematic footprint; review openings, heights, furniture and deletions below.</p></div>;
}

...
export default function MaterialEditorPanel({ isOpen, onClose, embedded = false }: { isOpen: boolean; onClose: () => void; embedded?: boolean }) {
  ...
  if (embedded) return <div className="material-editor-embedded flex h-full min-h-0 flex-col overflow-hidden bg-white text-zinc-900">
    ...
    <div className="min-h-0 flex-1 overflow-y-auto p-1.5 thin-scroll">
      ...
      <div className="mb-2 flex gap-1 overflow-x-auto pb-1 thin-scroll">{filtered.map((m) => <button key={m.id} draggable onDragStart={(e) => drag(e, m)} onClick={() => select(m.id)} className={`flex h-12 min-w-[108px] shrink-0 items-center gap-1.5 rounded-lg border px-1.5 text-left ${m.id === selected.id ? "border-amber-400 bg-amber-50" : "border-zinc-200 bg-white"}`}>{/* eslint-disable-next-line @next/next/no-img-element */}<img src={renderMaterialPreview(m, "sphere", 44)} alt="" className="h-9 w-9 shrink-0"/><span className="min-w-0"><span className="block truncate text-[9px] font-semibold">{m.name}</span><span className="block text-[8px] text-zinc-500">{m.category}</span></span></button>)}</div>
      <div className="grid grid-cols-[76px_1fr] gap-2 border-y border-zinc-200 py-2"><div className={`flex h-[76px] items-center justify-center rounded-lg border ${isDark ? "border-zinc-700 bg-zinc-950" : "border-zinc-200 bg-white"}`}>{/* eslint-disable-next-line @next/next/no-img-element */}<img src={renderMaterialPreview(selected, shape, 76)} alt="Preview" className="h-[70px] w-[70px]"/></div><div className="grid min-w-0 grid-cols-2 gap-1"><input value={selected.name} onChange={(e) => patch({ name: e.target.value })} className={`${field} col-span-2`}/><select value={selected.category} onChange={(e) => { const next = e.target.value as MaterialDefinition["category"]; patch({ ...CLASS_DEFAULTS[next], category: next }); }} className={field}>{CATEGORIES.slice(1).map((c) => <option key={c}>{c}</option>)}</select><input type="color" value={selected.color} onChange={(e) => patch({ color: e.target.value })} className="h-8 w-full rounded-lg border border-zinc-200 bg-white p-1"/><div className="col-span-2 flex gap-1">{SHAPES.map((s) => <button key={s.id} onClick={() => setShape(s.id)} title={s.label} className={`flex h-7 flex-1 items-center justify-center rounded-md border text-xs ${shape === s.id ? "border-amber-400 bg-amber-100" : "border-zinc-200"}`}>{s.icon}</button>)}</div></div></div>

...
export function getWelcomeLayout(width: number, height: number): WelcomeLayoutConfig {
  ...
  const mobileShared = {
    ...
    themeCardClass: `${cardBase} shrink-0 px-3 py-2.5`,
    seasonalCardClass: `${cardBase} shrink-0 px-3 py-2.5`,
    seasonalHintClass: "text-[10px] leading-snug text-[var(--text-muted)]",
    togglesRowClass: "",
    toggleCellClass: "",
    hintClass: "hidden",
    languageCompact: true as const,
    fieldsClass: "welcome-mobile-fields flex min-h-0 flex-1 flex-col gap-2 overflow-y-auto overscroll-contain",
    cardClass: `${cardBase} shrink-0 px-3 py-2.5`,
    inputClass: `${inputBase} h-10 shrink-0 px-3 py-2`,
    labelClass: "mb-1 block shrink-0 text-[11px] font-semibold tracking-wide text-[var(--text-muted)]",
    bodyTextClass: "text-xs font-medium text-[var(--text-body)]",
    actionsClass: "welcome-mobile-actions flex shrink-0 flex-row gap-2 pt-1",
    primaryBtnClass:
      "order-2 min-h-10 flex-1 font-semibold active:scale-[0.98] border border-amber-200/70 bg-gradient-to-br from-amber-200/95 via-yellow-300/85 to-amber-400/75 text-amber-950 shadow-[inset_0_1px_0_rgba(255,255,255,0.65),0_4px_14px_rgba(251,191,36,0.35)] backdrop-blur-md text-sm px-3 py-2",
    secondaryBtnClass:
      "order-1 min-h-10 flex-1 font-semibold active:scale-[0.98] border border-[var(--panel-divider)] bg-[var(--glass-inset-bg)] text-[var(--text-body)] shadow-[inset_0_1px_0_rgba(255,255,255,0.35)] backdrop-blur-md hover:border-[var(--glass-border)] text-xs px-3 py-2",
    contentBlockClass:
